...
`hermes claw migrate` 将你的 OpenClaw（或旧版 Clawdbot/Moldbot）配置导入 Hermes。本指南准确说明会迁移什么、配置键映射，以及迁移后要验证的内容。

:::note
改从 **Claude Code** 或 **OpenAI Codex CLI** 迁移？请使用 [`hermes import-agent`](../user-guide/import-from-other-agents.md)。
:::

:::tip
若你的 OpenClaw 配置使用多个 provider，`hermes setup --portal` 可将其整合为一个 OAuth——一次登录即可使用 300+ 个模型和 Tool Gateway。参见 [Nous Portal](/integrations/nous-portal)。
:::
...
迁移操作在执行任何更改前，始终会显示将导入内容的完整预览。请检查列表后确认继续。
...
工作区文件还会在 `workspace.default/` 和 `workspace-main/` 作为备用路径进行检查（OpenClaw 在近期版本中将 `workspace/` 重命名为 `workspace-main/`，多 Agent 配置下使用 `workspace-{agentId}`）。
...
技能冲突由 `--skill-conflict` 处理：`skip` 保留现有 Hermes 技能，`overwrite` 替换它，`rename` 创建带 `-imported` 后缀的副本。
...
| 自定义 provider | `models.providers.*` | `config.yaml` → `custom_providers` | 映射 `baseUrl`、`apiType`/`api`——同时处理短格式（"openai"、"anthropic"）和带连字符格式（"openai-completions"、"anthropic-messages"、"google-generative-ai"）值 |
...
| 人类延迟计时 | `agents.defaults.humanDelay.minMs` / `.maxMs` | `human_delay.min_ms` / `human_delay.max_ms` | 直接复制 |
...
| 工作目录 | `agents.defaults.workspace` | `config.yaml` → `terminal.cwd` | 旧版迁移仍可能将 `MESSAGING_CWD` 作为兼容性后备值输出 |

### 已归档（无直接对应的 Hermes 等效项）

这些内容保存至 `~/.hermes/migration/openclaw/<timestamp>/archive/` 供人工审查：
...
启用 `--migrate-secrets` 时，API 密钥按优先级从**四个来源**收集：
...
3. **配置 env 子对象** — `openclaw.json` → `"env"` 或 `"env"."vars"`（一些配置将密钥存储于此，而非单独的 `.env` 文件）
4. **认证档案** — `~/.openclaw/agents/main/agent/auth-profiles.json`（每个 agent 的凭据）

配置值优先级最高。每个后续来源填补剩余的空缺。
...
OpenClaw 配置中 token 和 API 密钥的值可以采用三种格式：
...
迁移会解析所有三种格式。对于环境模板和 `source: "env"` 的 SecretRef 对象，它会在 `~/.openclaw/.env` 和 `openclaw.json` env 子对象中查找该值。`source: "file"` 或 `source: "exec"` 的 SecretRef 对象无法自动解析——迁移会针对它们发出警告，且这些值必须通过 `hermes config set` 手动添加至 Hermes。

## 迁移后

1. **检查迁移报告** — 完成时会打印报告，其中包含已迁移、已跳过和冲突项的计数。

2. **审查归档文件** — `~/.hermes/migration/openclaw/<timestamp>/archive/` 中的任何内容都需要人工处理。

3. **开启新会话** — 导入的技能和记忆条目在新会话中生效，而不在当前会话中生效。

4. **验证 API 密钥** — 运行 `hermes status` 检查 provider 身份验证。

5. **测试消息功能** — 如果迁移了平台 token，请重启 gateway：`systemctl --user restart hermes-gateway`

6. **检查会话策略** — 运行 `hermes config show`，并确认 `session_reset` 值符合你的预期。
...
迁移依次检查 `~/.openclaw/`、`~/.clawdbot/`、`~/.moltbot/`。若你的安装路径在其他位置，请使用 `--source /path/to/your/openclaw`。

### "No provider API keys found"

根据 OpenClaw 版本不同，密钥可能存储在多个位置：`openclaw.json` 中 `models.providers.*.apiKey` 内联、`~/.openclaw/.env`、`openclaw.json` 的 `"env"` 子对象，或 `agents/main/agent/auth-profiles.json`。迁移会检查所有四个位置。若密钥使用 `source: "file"` 或 `source: "exec"` 的 SecretRef，则无法自动解析——请通过 `hermes config set` 添加它们。

### 迁移后技能未显示

导入的技能位于 `~/.hermes/skills/openclaw-imports/`。开启新会话后它们才会生效，或运行 `/skills` 验证它们是否已加载。

### TTS 语音未迁移

OpenClaw 在两个位置存储 TTS 设置：`messages.tts.providers.*` 和顶层 `talk` 配置。迁移会同时检查两处。若你的语音 ID 是通过 OpenClaw UI 设置的（存储在不同路径），可能需要手动设置：`hermes config set tts.elevenlabs.voice_id YOUR_VOICE_ID`。
