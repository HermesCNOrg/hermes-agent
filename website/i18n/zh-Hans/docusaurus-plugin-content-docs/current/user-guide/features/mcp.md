...
- **代理回调（`redirect_uri`）：** 当公共 HTTPS 端点转发到该主机（例如指向回调端口的 Tailscale Funnel 或反向代理）时，设置 `oauth.redirect_uri`，浏览器重定向便会自行到达 Hermes——无需隧道或粘贴：
...
对于完全无头的 gateway（消息机器人，完全没有交互式终端），可选的 [`mcp-oauth-remote-gateway` skill](../skills/optional/mcp/mcp-mcp-oauth-remote-gateway.md) 会引导 agent 手动完成该流程，并将令牌写入 Hermes 预期的位置。

**陷阱——WAF 拒绝 `127.0.0.1` 重定向 URI。** 少数提供商在其授权服务器前部署了 WAF，凡是查询字符串中包含字面量 `127.0.0.1` 的授权请求都会返回 403（Reclaim.ai 的 AWS API Gateway 是已知示例——每次尝试均在到达 OAuth 应用之前返回 `{"message":"Forbidden"}`）。请设置 `oauth.redirect_host: localhost` 以改用 `http://localhost:<port>/callback`；无论哪种方式，回调监听器仍绑定到 `127.0.0.1`。
...
两个列表除精确名称外，也接受 fnmatch 风格的 glob——这对于 Cloudflare 的 API MCP（`?codemode=false`，约 3,300 个工具）等庞大的扁平接口至关重要；在这类接口中，逐个端点排除产品区域并不现实：
...
不含 glob 元字符（`*`、`?`、`[`）的条目会精确匹配——`docs` 只排除名为 `docs` 的工具，绝不会排除 `docs_search`。
