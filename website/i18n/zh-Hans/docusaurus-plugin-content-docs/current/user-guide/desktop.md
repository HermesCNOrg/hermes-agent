...
- **输入历史与队列编辑**：输入框为空时按上/下方向键，可调出并复用之前的提示词；还可在已排队消息发送前进行编辑。排队期间按“停止”（或 Esc）会暂停队列，并将其展开到输入框上方；你可以从那里恢复队列，也可以逐条发送、编辑或删除。
...
Hermes Desktop 会扫描你的家目录（深度受限），以便在 Projects 侧边栏中发现本地 Git 仓库。你可以按 profile 在**设置 → 工作区**中修改此行为，或在 `config.yaml` 中配置：
...
更改其中任一值只会使该 profile 的磁盘发现缓存失效，并启动符合新策略的刷新。**从侧边栏隐藏**仍是独立的逐项整理操作。
