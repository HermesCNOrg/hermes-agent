...
:::tip 首次设置
一条命令——`hermes setup --portal`——即可让你准备好使用 `hermes chat`。参见 [Nous Portal](/integrations/nous-portal)。
:::
...
在 `openai-codex` 提供商上，`/usage` 还会显示 ChatGPT 帐户中任何已储备的使用限制重置（“You have N resets banked - use /usage reset to activate”）。`/usage reset` 会兑换一次已储备的重置，完全恢复你的 5 小时和每周限制。Hermes 会在你的限额尚未耗尽时拒绝兑换（一次已储备重置会恢复完整额度，因此过早使用会造成浪费）——传入 `/usage reset --force` 仍可强制兑换。
...
| `/context [all]` | 可视化上下文使用情况分解——字形块网格 + 按类别划分的 token 表（系统提示词 / 工具 / skill / 记忆 / 对话 / 可用空间）。`/context all` 会增加每个 skill 和每个工具集的开销。 |
...
当 agent 正在工作时，你可以发送一条修正信息，而无需开始新的轮次：

- **输入一条新消息 + Enter** —— 使用你的修正信息重定向活跃轮次
- **`Ctrl+C`** —— 中断当前操作（2 秒内连按两次可强制退出）
- 已显示的已完成工具工作和推理会保留在上下文中
- 正在运行的工具会到达其安全边界后再应用修正
...
| 模式 | 行为 |
|------|----------|
| `"interrupt"`（默认） | 你的消息重定向活跃轮次。模型生成会重新开始，并保留已显示的推理和已完成的工作；正在运行的工具先完成 |
...
`"queue"` 模式会准备一个独立的后续轮次。`"steer"` 始终等待下一个工具结果边界。默认的 `"interrupt"` 模式会在模型生成期间更快响应，同时避免取消正在运行的工具。希望取消该轮次及其前台工作时，请使用 `/stop`。未知值会回退到 `"interrupt"`。
...
:::tip 首次触发提示
第一次在 Hermes 工作时按下 Enter，Hermes 会打印一行提醒，说明 `/busy` 选项。每次安装只触发一次；`config.yaml` 中的 `onboarding.seen.busy_input_prompt` 会记录其已显示。删除该键可再次看到提示。
:::
