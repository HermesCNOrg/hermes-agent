...
| `--tui` | 启动 [TUI](../user-guide/tui.md) 而非经典 CLI。等同于 `HERMES_TUI=1`，且始终优先于 `display.interface`。 |
| `--cli` | 强制使用经典 prompt_toolkit REPL。可用它在单次调用中覆盖 `display.interface: tui`。 |
...
| `hermes moa` | 配置可从模型选择器中选用的命名 Mixture of Agents 预设。 |
...
| `hermes egress` | 用于远程终端沙箱的出站凭据注入防火墙（iron-proxy）。默认禁用。参见 [Egress 代理](../user-guide/egress/iron-proxy.md)。 |
...
| `hermes whatsapp-cloud` | 配置官方 Meta WhatsApp Business Cloud API 适配器（需要 Business 账号和公网 webhook）。它不同于 `hermes whatsapp`（基于 Baileys 的个人账号桥接）。 |
...
| `hermes send` | 向已配置的消息平台（Telegram、Discord、Slack、Signal、SMS 等）发送单次消息。适合 shell 脚本、cron 任务、CI hook 和监控 daemon——不启动 agent 循环，也不调用 LLM。 |
| `hermes secrets` | 管理外部 secret 来源（目前支持 Bitwarden Secrets Manager），在进程启动时拉取 API 密钥，而不是从 `~/.hermes/.env` 读取。 |
...
| `hermes project` | 管理命名的多文件夹工作区（project）。它为桌面端会话分组提供锚点；绑定 kanban 看板后，还会为任务提供确定性的 worktree + branch 约定。状态按 profile 隔离。 |
...
| `hermes prompt-size` | 显示 system prompt + tool schema 的字节明细（skill 索引、memory、profile）。完全离线运行。 |
...
| `hermes pets` | 浏览、安装和选择 [petdex](../user-guide/features/pets.md) 动画宠物；它们会显示在 CLI、TUI 和桌面应用中。子命令：`list`、`install`、`select`、`show`、`off`、`scale`、`remove`、`doctor`。 |
...
| `--safe-mode` | 故障排查模式：禁用所有自定义内容——用户配置、规则/memory 注入、plugin、shell hook 和 MCP 服务器（隐含 `--ignore-user-config` 和 `--ignore-rules`）。用于判断问题来自你的设置还是 Hermes 本身。 |
...
使用 `--global` 切换时，provider 和 base URL 的更改会与模型一起持久化到 `config.yaml`。从自定义端点切换走时，过时的 base URL 会被清除，以防止其泄漏到其他 provider。
...
| `uninstall` | 删除已安装的服务。 |
| `setup` | 交互式消息平台设置。 |
| `migrate-legacy` | 删除重命名前安装遗留的旧 `hermes.service` unit。绝不会触及 profile unit（`hermes-gateway-<profile>.service`）或无关服务。标志：`--dry-run`、`-y`/`--yes`。 |
| `enroll` | 实验性功能：向 relay connector 注册此 gateway，并为由 connector 支持的平台保存 relay 凭据。 |
...
| `--external-supervisor` | 在 `run` 时：声明由封装程序提供的进程管理器负责管理前台 gateway。当 `sudo`、`env -i` 或其他封装程序移除了 launchd/systemd 的原生环境标记时使用。聊天内重启和更新会退出并返回该管理器，而不是生成脱离控制的替代进程。 |

`--external-supervisor` 是一项重启策略约定：聊天内重启或服务重启更新会以状态码 `75` 退出，因此封装程序的 supervisor 必须在该非零退出后重新启动 gateway。对于 systemd，请使用 `Restart=on-failure` 或 `Restart=always`，且不要在 `RestartPreventExitStatus` 中包含 `75`；对于 launchd，请配置 `KeepAlive`，使其在非成功退出后重新启动。如果没有此策略，请求重启会使 gateway 保持停止状态。

`hermes gateway enroll` 接受 `--token`、`--connector-url`、`--gateway-id` 和 `--wake-url`。它会使用注册 token 与 connector 交换凭据，并将得到的 `GATEWAY_RELAY_ID`、`GATEWAY_RELAY_SECRET`、`GATEWAY_RELAY_DELIVERY_KEY`、可选的 `GATEWAY_RELAY_URL`，以及（提供 `--wake-url` 时）`GATEWAY_RELAY_WAKE_URL` 写入当前 profile 的 `.env`。
...
```
**最简便的方式：** `hermes setup --portal`——通过 OAuth 登录 Nous Portal，并一次性选择加入 [Tool Gateway](../user-guide/features/tool-gateway.md)。
...
| `--long-description-file PATH` | 未设置 | 从 UTF-8 文本文件读取长描述，并精确保留其内容。与 `--long-description` 互斥，且与 `--slashes-only` 不兼容。 |
...
```
无需启动 agent 或 gateway 循环，即可向已配置的消息平台发送单次消息。它复用 gateway 中已有的凭据（`~/.hermes/.env` + `~/.hermes/config.yaml`），因此运维脚本、cron 任务、CI hook 和监控 daemon 无需重新实现各平台的 REST 客户端，就能发送状态更新。

对于使用 bot token 的平台（Telegram、Discord、Slack、Signal、SMS、WhatsApp-CloudAPI），无需运行 gateway——`hermes send` 会直接调用平台的 REST 端点。需要常驻适配器的 plugin 平台仍要求 gateway 处于运行状态。
...
| `-f`, `--file <PATH>` | 从 `PATH` 读取消息正文（仅限日志、报告、Markdown 等文本文件）。传入 `-` 可强制从 stdin 读取。要发送图片或其他二进制文件，请使用 `MEDIA:<path>`（见下文）。 |
| `-s`, `--subject <LINE>` | 在消息正文前添加主题/标题行。 |
| `-l`, `--list [platform]` | 列出所有平台（或仅指定平台）中已配置的目标。 |
| `-q`, `--quiet` | 成功时不向 stdout 输出内容——适合仅依赖退出码的脚本。 |
| `--json` | 输出原始 JSON 结果，而不是便于阅读的文本。 |

如果既未提供位置参数 `message`，也未提供 `--file`，且 stdin 不是 TTY，`hermes send` 会从 stdin 读取。退出码：成功为 `0`，投递/后端失败为 `1`，用法错误为 `2`。

### 发送图片和其他媒体

`--file` 仅用于*文本*正文。要将图片、文档、视频或音频文件作为平台原生附件发送，请在消息文本中使用 `MEDIA:<local_path>` 指令引用它：
...
```
默认情况下，图片文件会作为照片发送（Telegram 等平台会重新压缩）。在消息中加入 `[[as_document]]`，即可改为不压缩的文件附件：
...
```
在进程启动时从外部 secret manager 拉取 API 密钥，而不是将其存储在 `~/.hermes/.env` 中。目前支持 **Bitwarden Secrets Manager**。完整指南请参阅：[Bitwarden 集成](../user-guide/secrets/bitwarden.md)。

`bitwarden`（别名 `bw`）子命令：

| 子命令 | 说明 |
|------------|-------------|
| `setup` | 交互式向导：安装固定版本的 `bws` 二进制文件、保存 access token 并选择 project。非交互式使用时可传入 `--project-id`、`--access-token` 和 `--server-url`。 |
| `status` | 显示当前配置、二进制路径/版本及 token 验证状态。 |
| `token` | 轮换 access token：在保存到 `.env` 前，先通过 Bitwarden 验证新 token（被拒绝的 token 不会改变任何内容）。非交互式使用时接受 `--access-token`，使用 `--no-verify` 可跳过探测。 |
| `sync` | 立即拉取 secret 并报告变更。添加 `--apply` 才会真正将 secret 导出到当前 shell 环境中（默认仅预览）。 |
| `install` | 下载并验证固定版本的 `bws` 二进制文件。即使已有托管副本，`--force` 也会重新下载。 |
...
hermes migrate <type>
```
诊断并（可选）重写当前 `config.yaml`，替换已退役模型或弃用设置的引用。任何重写前都会对原始 `config.yaml` 创建带时间戳的备份（可用 `--no-backup` 跳过）。

| 子命令 | 说明 |
|------------|-------------|
| `xai` | 扫描 `config.yaml` 中计划于 2026 年 5 月 15 日退役的 xAI 模型引用，并在使用 `--apply` 时按照 xAI 迁移指南原地改写为官方替代模型。默认仅预览。 |

迁移子命令的通用标志：

| 标志 | 说明 |
|------|-------------|
| `--apply` | 原地重写 `config.yaml`（默认仅预览，不写入）。 |
| `--no-backup` | 应用变更时跳过 `config.yaml` 的带时间戳备份。 |

> 请勿与 `hermes claw migrate`（将 OpenClaw 配置一次性导入 Hermes）混淆——`hermes migrate` 是顶级配置重写命令。
...
```
运行本地 OpenAI 兼容 HTTP 服务器，将请求转发到通过 OAuth 认证的上游 provider（例如 Nous Portal、xAI）。外部应用可使用任意 bearer token 指向此代理；代理会在转发时附加你真实的 OAuth 凭据。完整指南请参阅[订阅代理](../user-guide/features/subscription-proxy.md)。
...
```
通过 [OSV.dev](https://osv.dev) 按需扫描漏洞。覆盖 Hermes venv（已安装的 PyPI distribution）、`~/.hermes/plugins/` 下 plugin 声明的 Python 依赖，以及 `config.yaml` 中固定版本的 `npx`/`uvx` MCP 服务器。不扫描全局安装的软件包或编辑器/浏览器扩展。
...
| 标志 | 默认值 | 说明 |
|------|---------|-------------|
| `--json` | 关闭 | 输出机器可读的 JSON，而非便于阅读的文本。 |
| `--fail-on <level>` | `critical` | 任何发现达到此严重级别时以非零状态退出（`low`、`moderate`、`high`、`critical`）。 |
| `--skip-venv` | 关闭 | 跳过扫描 Hermes Python venv。 |
| `--skip-plugins` | 关闭 | 跳过扫描 plugin requirements 文件。 |
| `--skip-mcp` | 关闭 | 跳过扫描 `config.yaml` 中固定版本的 MCP 服务器。 |
...
cron **trigger** 可通过 `cron.provider` 配置键替换。留空（默认）时使用内置的进程内 ticker。可将其设为 `chronos`（由 NAS 管理、用于可缩容至零的托管 gateway），并通过 `cron.chronos.*` 键（`portal_url`、`callback_url`、`expected_audience`、`nas_jwks_url`）配置；也可指定 `plugins/cron/<name>/` 或 `$HERMES_HOME/plugins/<name>/` 下的自定义 provider。未知或不可用的 provider 会回退到内置实现，因此 cron 始终有可用 trigger。详见 [cron 内部原理](../developer-guide/cron-internals.md#gateway-integration)。
...
| `decompose <id>` / `decompose --all` | 将 triage 列中的任务按描述拆分为子任务图，并路由到专业 profile。当 LLM 判断任务不适合拆分时，回退到 specify 风格的单任务提升。标志与 `specify` 相同。在 `config.yaml` 的 `auxiliary.kanban_decomposer` 下配置拆分模型；`kanban.orchestrator_profile` 仅控制拆分后根任务/编排任务的归属。当 `kanban.auto_decompose: true`（默认）时，每次调度器 tick 也会自动运行。参见 [自动与手动编排](/user-guide/features/kanban#auto-vs-manual-orchestration)。 |
...
用于远程终端沙箱的出站凭据注入防火墙。它封装了 [iron-proxy](https://github.com/ironsh/iron-proxy) daemon——这是一个执行 TLS 拦截的代理，会在网络边界将不透明代理 token 替换为真实的上游 API 凭据，因此沙箱永远不会持有真实密钥。默认禁用；设置和架构请参阅完整的 [Egress 代理](../user-guide/egress/iron-proxy.md)页面。
...
```
常见故障模式和恢复方法见 [Egress 代理 → 故障排查](../user-guide/egress/iron-proxy.md#troubleshooting)。
...
```
Project 是用户命名的工作区，可以跨多个文件夹/仓库。它们为桌面端会话分组提供锚点；绑定 kanban 看板后，还会为任务提供确定性的 worktree + branch 约定。状态按 profile 隔离。
...
| `remove-folder` | 从 project 删除文件夹。 |
| `rename` | 重命名 project。 |
| `set-primary` | 设置主文件夹。 |
| `use` | 设置当前 project。 |
| `archive` | 归档 project（可恢复）。 |
| `restore` | 恢复已归档的 project。 |
| `bind-board` | 将 kanban 看板绑定到此 project。 |
...
| `--nous` | 上传到 Nous 内部诊断存储，而不是公共粘贴服务。当 Nous 支持人员要求提供私有诊断包时使用。 |
| `--local` | 在本地打印报告而非上传。 |
| `--no-redact` | 禁用上传时的 secret 脱敏。默认会对上传内容进行脱敏。 |

报告包含系统信息（操作系统、Python 版本、Hermes 版本）、近期 agent、gateway、GUI/dashboard 和桌面端日志（每文件上限 512 KB），以及脱敏的 API 密钥状态。默认会对上传内容进行脱敏，因此不包含 secret。

默认按顺序尝试公共粘贴服务：paste.rs、dpaste.com。`--nous` 则将同一调试包上传到 Nous 私有诊断存储；返回的查看链接供 Nous 团队使用，并会在 14 天后自动删除。
...
| `gui` | `gui.log` | Dashboard / TUI gateway / PTY bridge / websocket 事件 |
| `desktop` | `desktop.log` | Electron 桌面应用——启动、后端进程输出和近期 Python traceback |
...
```
报告新会话的固定 prompt 预算——即加入任何对话内容之前，每次 API 调用都会发送的内容。当下游适配器或代理的 prompt 预算小于模型 context window，或你想查看哪个区块（skill 索引、memory、profile）占用最多空间时，这项功能很有用。

它会构建与 agent 实际使用相同的 system prompt，然后拆分为：

- **System prompt 总计**——完整组装后的 prompt（身份、指引、skill 索引、context 文件、memory、profile、时间戳）。
- **Skill 索引**——`<available_skills>` 区块。安装大量 skill 时，这通常是最大的单一区块。
- **Memory** 和**用户 profile**——你的 `MEMORY.md` / `USER.md` 快照。
- **Prompt 分层**——stable / context / volatile，与 Hermes 为提高缓存友好性而采用的 prompt 分层方式一致。
- **Tool schema**——所有已启用工具的 JSON（每次调用固定 payload 的另一半）。

完全离线运行——不会调用 API，未配置凭据也可使用。
...
```
:::tip
Skill 索引和 tool schema 的大小会随已安装 skill 和已启用工具的数量增长。要缩小 prompt，请禁用未使用的 toolset（`hermes tools`）或卸载不需要的 skill（`hermes skills`）。当前目录中的 context 文件（AGENTS.md、.cursorrules）也会计入总量。
:::
...
| `opt-out` | 停止向当前 profile 植入捆绑 skill。写入 `.no-bundled-skills` 标记，使安装程序、`hermes update` 和所有同步操作都跳过捆绑 skill 植入。默认安全，不会修改磁盘上的现有 skill。使用 `--remove` 时，还会删除已有但**未经修改**的捆绑 skill（用户编辑、hub 安装和手写的 skill 永远不会删除；操作前会预览并确认，`--yes` 可跳过确认）。 |
| `opt-in` | 删除 `.no-bundled-skills` 标记以撤销 `opt-out`，让下次 `hermes update` 再次植入捆绑 skill。使用 `--sync` 可立即重新植入。 |
...
配置命名的 Mixture of Agents 预设。预设会在每个模型选择器中显示为 `Mixture of Agents` provider 下的可选模型；`/moa <prompt>` 会使用默认预设运行一次 prompt。
...
```
`hermes moa configure` 会为每个参考模型和 aggregator 复用 Hermes 的 provider → 模型选择器。预设是一种执行模式配置，不是主模型或 provider。
...
| 子命令 | 说明 |
|------------|-------------|
| *（无）* 或 `picker` | 交互式目录选择器——浏览 Nous 审核的 MCP，并安装/启用/禁用。 |
| `catalog` | 列出 Nous 审核的 MCP（纯文本，可用于脚本）。 |
| `install <name>` | 安装目录条目（例如 `hermes mcp install n8n`）。 |
| `serve [-v\|--verbose]` | 将 Hermes 作为 MCP 服务器运行——向其他 agent 暴露对话。 |
| `add <name> [--url URL] [--command CMD] [--auth oauth\|header] [--args ...]` | 添加自定义 MCP 服务器并自动发现工具。`--args` 会将剩余 argv 传给 stdio 命令，因此应放在最后。 |
...
```
[Petdex](https://github.com/crafter-station/petdex) 是面向 coding agent 的公开动画 sprite 宠物图库。安装宠物后，Hermes 会在 CLI、TUI 和桌面应用中显示它，并让它随 agent 活动做出反应。

| 子命令 | 说明 |
|------------|-------------|
| `list` | 浏览 petdex 图库。 |
| `install` | 从图库安装宠物。 |
| `select` | 设置当前宠物（写入 `display.pet.*`）。 |
| `show` | 在终端中播放当前宠物的动画。 |
| `off` | 禁用宠物显示。 |
| `scale` | 在所有界面调整宠物大小（`display.pet.scale`）。 |
| `remove` | 删除已安装的宠物。 |
| `doctor` | 检查宠物设置和终端图形支持。 |

你还可以使用 `/hatch` 斜杠命令，根据文本描述生成全新的宠物。参见 [Pets](../user-guide/features/pets.md)。
...
| `prune` | 删除符合过滤条件的会话：时间范围 `--older-than`/`--newer-than`/`--before`/`--after`（支持 `5h`/`2d` 等 duration、纯天数或 ISO 时间戳）；属性 `--source`、`--title`、`--model`、`--provider`、`--branch`、`--end-reason`、`--user`、`--chat-id`、`--chat-type`、`--cwd`；数值范围 `--min/--max-messages`、`--min/--max-tokens`、`--min/--max-cost`、`--min/--max-tool-calls`；另支持 `--include-archived`、`--dry-run`、`--yes`。默认删除 90 天前的会话。 |
| `archive` | 批量归档（软隐藏，不删除）符合与 `prune` 相同过滤条件的会话。至少需要提供一个过滤条件。 |
| `stats` | 显示会话存储统计信息。 |
...
```
将 **Claude Code**（`~/.claude`）或 **OpenAI Codex CLI**（`~/.codex`）设置导入 Hermes。它把 `CLAUDE.md`/`AGENTS.md` 指令映射到 memory 条目，把 `Bash(...)` 权限允许/拒绝规则映射到 `command_allowlist`/`approvals.deny`，把 MCP 服务器映射到 `config.yaml` 中的 `mcp_servers`，并把 skill 目录导入 `~/.hermes/skills/`。应用前始终会先预览；绝不会导入 API 密钥和凭据。
...
```
启动 Hermes **后端服务器**——[桌面应用](/user-guide/desktop)和远程客户端连接的 JSON-RPC/WebSocket gateway。它与 `hermes dashboard` 运行的是同一服务器，但采用**无界面模式**：绝不会打开浏览器 UI。桌面应用会启动自己的 `hermes serve` 后端；需要在远程主机上运行无界面后端时，可直接使用此命令。它接受下方 `hermes dashboard` 相同的 `--host` / `--port` / `--insecure` / `--skip-build` / `--stop` / `--status` 选项（绑定到非 loopback 地址时会启用相同的认证 gate）。需要 `[web]` extra；内嵌 Chat socket 在 POSIX 主机上还需要 `[pty]`。
...
```
启动 Web 控制台——基于浏览器的界面，用于管理配置、API 密钥和监控会话。（如果只需没有浏览器 UI 的后端，例如桌面应用启动的后端，请使用上方的 [`hermes serve`](#hermes-serve)。）需要 `cd ~/.hermes/hermes-agent && uv pip install -e ".[web]"`（FastAPI + Uvicorn）。内嵌浏览器 Chat 标签页始终可用，但额外需要 `pty` extra（`cd ~/.hermes/hermes-agent && uv pip install -e ".[web,pty]"`）以及 POSIX PTY 环境（如 Linux、macOS 或 WSL2）。完整文档请参阅 [Web 控制台](/user-guide/features/web-dashboard)。
...
| `--host` | `127.0.0.1` | 绑定地址 |
| `--no-open` | — | 不自动打开浏览器 |
| `--insecure` | 关闭 | **已弃用/无操作。** 过去可在绑定非 loopback 地址时绕过认证。自 2026 年 6 月安全加固后，公网绑定*始终*需要认证 provider（密码或 OAuth）。如需保持本地访问，请绑定 `127.0.0.1` 并使用隧道。 |
| `--skip-build` | 关闭 | 跳过 Web UI 构建，直接提供已有的 `dist`。适用于 npm 不可用的非交互式环境（Windows Scheduled Tasks、CI）。请先运行 `cd web && npm run build` 完成构建。 |
| `--isolated` | 关闭 | 从命名 profile（`worker dashboard`）启动时，运行该 profile 专用的服务器，而不是路由到整机 dashboard。 |
| `--stop` | — | 停止正在运行的 `hermes dashboard` 进程并退出。 |
| `--status` | — | 列出正在运行的 `hermes dashboard` 进程并退出。 |

### `hermes dashboard register`

将此安装注册为与你的 Nous Portal 账号关联的自托管 dashboard。该命令会创建 OAuth client，将 `HERMES_DASHBOARD_OAUTH_CLIENT_ID` 写入 `~/.hermes/.env`，并打印启用登录 gate 的方法。要求已登录（`hermes setup`）。

| 选项 | 说明 |
|--------|-------------|
| `--name` | Dashboard 便于阅读的标签（默认自动生成）。 |
| `--redirect-uri` | 公网 HTTPS OAuth redirect URI（例如 `https://hermes.example.com/auth/callback`）。仅限 localhost 使用时可省略。 |
| `--portal-url` | 覆盖注册所用的 Nous Portal base URL（默认为你登录的 Portal）。也可通过 `HERMES_DASHBOARD_PORTAL_URL` 设置。 |
...
`hermes update` 会拉取已配置的更新 branch（默认：`main`）。如果当前检出位于其他 branch，Hermes 可能会先切换到更新 branch 再拉取。如果你希望 branch 上的工作不进入更新时的自动 stash 流程，请先提交这些变更。

| 选项 | 说明 |
|--------|-------------|
| `--gateway` | 消息 `/update` 命令使用的内部模式。它通过基于文件的 IPC 处理提示和进度流，而不是从终端 stdin 读取。它不是 gateway 重启标志。 |
| `--check` | 仅检查是否有可用更新，不拉取、不安装依赖，也不重启任何内容。 |
| `--no-backup` | 本次运行跳过所有更新前备份（包括快速状态快照和完整 zip），无论 `updates.pre_update_backup` 的值如何。 |
| `--backup` | 强制本次运行执行**完整**更新前备份：创建快速状态快照以及包含 `HERMES_HOME`（config、auth、会话、skill、配对数据）的完整 zip。默认模式为 `quick`——仅创建轻量级状态快照。可在 `config.yaml` 中通过 `updates.pre_update_backup: quick | full | off` 设置永久模式。 |
| `--yes`, `-y` | 对 config 迁移和 stash 恢复等交互式提示自动回答 yes。API 密钥输入会跳过；请另行运行 `hermes config migrate`。 |

附加行为：

- **Gateway 重启。** 更新成功后，Hermes 会尝试自动重启所有正在运行的 gateway profile，使其加载新代码。如果只想重启 gateway 而不更新，请使用 `hermes gateway restart`。
- **本地源码变更。** 对 git 安装，切换 branch 或拉取前会自动 stash 已修改的 tracked 文件和 untracked 文件（`git stash push --include-untracked`）。交互式终端更新会在恢复 stash 前询问；非交互式更新默认恢复。只有在由平台管理、且成功拉取后应丢弃本地源码修改的安装中，才应设置 `updates.non_interactive_local_changes: discard`。若恢复 stash 发生冲突或拉取失败，stash 会保留以供手动恢复。
- **npm lockfile 变动。** 在 stash 或切换 branch 前，Hermes 会尽力清理 npm install/build 产生的 tracked `package-lock.json` diff。运行 `hermes update` 前，请提交或手动 stash 有意的 lockfile 修改。
...
| `hermes uninstall [--full] [--gui] [--yes]` | 删除 Hermes，可选择删除所有 config/数据。`--gui` 仅删除桌面 Chat GUI，保留 agent；`--full` 还会删除 config/数据；`--yes` 跳过提示。 |
